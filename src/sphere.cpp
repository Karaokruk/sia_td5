...
void Sphere::init()
{
    ...
    vertices.resize(3*sphere_points.size());
    for(unsigned int i=0; i<sphere_points.size(); i++){
    ...
    m_pMesh->loadRawData((float*)vertices.data(), sphere_points.size(), (int*)&indices[0], indices.size()/3);
}
...
bool Sphere::intersect(const Ray& ray, Hit& hit) const
{
    ...
    if(discr>=0)
    {
        ...
        longitude = 1.f - (longitude + M_PI/2.f) / (2.f*M_PI);
        latitude = fabs((latitude + M_PI/2.f) / M_PI);
