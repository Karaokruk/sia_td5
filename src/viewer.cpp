...
void Viewer::drawContents()
{
    ...
    else if(m_scene) // scene loaded, OpenGL rendering
    {
        ...
        {
            for(unsigned int i=0; i<m_scene->shapeList().size(); ++i)
            {
...
void renderBlock(Scene* scene, ImageBlock& block)
{
    ...
    float tanfovy2 = tan(camera->fovY()*0.5f);
    ...
    for (int y=0; y<size.y(); ++y) {
        for (int x=0; x<size.x(); ++x) {
            ...
            ray.direction = (camF + camX * (2.f*float(x + 0.5f + offset[0])/float(camera->vpWidth())  - 1.f)
                                  - camY * (2.f*float(y + 0.5f + offset[1])/float(camera->vpHeight()) - 1.f)).normalized();
...
void render(Scene* scene, ImageBlock* result, bool* done)
{
    if(!scene)
        return;
    ...
    if(action == GLFW_PRESS) {
        ...
        case GLFW_KEY_R:
        {
            if(m_scene && m_renderingDone) {
                ...
                std::thread render_thread(render,m_scene,m_resultImage,&m_renderingDone);
